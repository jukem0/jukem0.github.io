...
// move bad left
badXPosition += badXSpeed;

// keep bad to the right in menu
if(gameState == "menu") {
    badXPosition = spawnXPosition;
}

// respawn bad when it falls off screen
if(badXPosition < -100 && gameState == "action") {
    badXPosition = spawnXPosition;
    badYPosition = randomBetween(0, canvas.height);
}

// check if the bad and the bird collide
if(theseCirclesCollide(
    birdXPosition,
    birdYPosition,
    birdHitboxRadius,
...
{ // if they do, increase the score and respawn bad
    badSound.play();
    scoreValue += badValue;
    badXPosition = spawnXPosition;
    badYPosition = randomBetween(0, canvas.height);
