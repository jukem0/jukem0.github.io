...
for(let bad of bads) {
    ...
    if(debugModeIsOn) {
        drawCircle(
          ...
          // move the bad
          bad.xPosition += badXSpeed;


        if(gameState == "action") {
            // check if the bads collides with the bird
            if(theseCirclesCollide(
                birdXPosition,
                birdYPosition,
                birdHitboxRadius,
            ...
            { // if they do, increase the score
                badSound.play();
                scoreValue += badValue;
                bads = bads.remove(bad);
            }
        }

         // remove bad if it goes off the screen
         if(bad.xPosition < destructionXPosition) {
            bads = bads.remove(bad);
        }
    }
    ...
    // spawn new bads
    if(gameState == "action" &&
    badTimeSinceLastSpawn>badSpawnInterval) {
        bads.push({
            xPosition: spawnXPosition,
            yPosition: randomBetween(0, canvas.height)
        });
        badTimeSinceLastSpawn = 0;
    }

    if(gameState == "action") {
        badTimeSinceLastSpawn += timeSinceLastFrame;
    }
